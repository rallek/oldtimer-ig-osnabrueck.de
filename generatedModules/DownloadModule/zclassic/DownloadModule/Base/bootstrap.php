 ...
 */
downloPerformRegularAmendments();

/**
 * Performs regular amendments, like archiving obsolete data.
 */
function downloPerformRegularAmendments()
{
    ...
    PageUtil::registerVar('RKDownLoadModuleAutomaticArchiving', false, true);
    $serviceManager = ServiceUtil::getManager();
    // check if own services exist (which is not true if the module is not installed yet)
    if (!$serviceManager->has('rk_download_module.workflow_helper')) {
        return;
    }
    $workflowHelper = $serviceManager->get('rk_download_module.workflow_helper');

    if (!$serviceManager->has('rk_download_module.hook_helper')) {
        return;
    }
    $hookHelper = $serviceManager->get('rk_download_module.hook_helper');
    ...
    if (!$serviceManager->has('rk_download_module.file_factory')) {
        return;
    }

    // perform update for files becoming archived
    $logger->notice('{app}: Automatic archiving for the {entity} entity started.', ['app' => 'RKDownLoadModule', 'entity' => 'file']);
    $repository = $serviceManager->get('rk_download_module.file_factory')->getRepository();
    $repository->archiveObjects($permissionApi, $session, $translator, $workflowHelper, $hookHelper);
    $logger->notice('{app}: Automatic archiving for the {entity} entity completed.', ['app' => 'RKDownLoadModule', 'entity' => 'file']);
    PageUtil::setVar('RKDownLoadModuleAutomaticArchiving', false);
}
