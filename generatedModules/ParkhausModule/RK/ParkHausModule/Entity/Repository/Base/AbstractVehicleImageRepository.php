 ...
 *
 * This is the base repository class for vehicle image entities.
 */
abstract class AbstractVehicleImageRepository extends EntityRepository
{
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $logArgs = ['app' => 'RKParkHausModule', 'entity' => 'vehicle image'];
     ...
     *
     * @return array|vehicleImageEntity retrieved data array or vehicleImageEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection collection containing retrieved vehicleImageEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = [])
    {
        ...
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved vehicle images only
            $onlineStates = ['approved'];
            
            $showOnlyOwnEntries = $this->getRequest()->query->getInt('own', 0);
            if ($showOnlyOwnEntries == 1) {
                // allow the owner to see his deferred vehicle images
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numVehicleImages';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('RK\ParkHausModule\Entity\VehicleImageEntity', 'tbl');
     ...
     * @param int    $excludeId  Id of vehicle images to exclude (optional)
     *
     * @return boolean result of this check, true if the given vehicle image does not already exist
     */
    ...
    {
        $qb->leftJoin('tbl.vehicle', 'tblVehicle');
