...
namespace RK\ParkHausModule\Entity\Repository\Base;
 ...
 *
 * This is the base repository class for vehicle entities.
 */
abstract class AbstractVehicleRepository extends EntityRepository
{
    ...
    {
        $fieldName = 'vehicleDescriptionTeaser';
    ...
    public function getAdditionalTemplateParameters(ImageHelper $imageHelper, $context = '', $args = [])
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], ['display', 'view'])) {
                ...
                $templateParameters['relationThumbRuntimeOptions'] = $imageHelper->getCustomRuntimeOptions('', '', 'RKParkHausModule_relateditem', $context, $args);
            }
        }
    ...
    protected function getViewQuickNavParameters($context = '', $args = [])
    {
        ...
        $parameters['vehicleType'] = $this->getRequest()->query->get('vehicleType', '');
        $parameters['owner'] = (int) $this->getRequest()->query->get('owner', 0);
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $logArgs = ['app' => 'RKParkHausModule', 'entity' => 'vehicle'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $logArgs = ['app' => 'RKParkHausModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'vehicles', 'userid' => $userId];
    ...
    public function updateUserField($userFieldName, $userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        if (empty($userFieldName) || !in_array($userFieldName, ['owner'])) {
        ...
        $logArgs = ['app' => 'RKParkHausModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'vehicles', 'field' => $userFieldName, 'userid' => $userId, 'newuserid' => $newUserId];
    ...
    public function deleteByUserField($userFieldName, $userId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $logArgs = ['app' => 'RKParkHausModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'vehicles', 'field' => $userFieldName, 'userid' => $userId];
     ...
     *
     * @return array|vehicleEntity retrieved data array or vehicleEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection collection containing retrieved vehicleEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.vehicleType = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.titleImage = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.copyrightTitleImage LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.vehicleImage = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.copyrightVehicleImage LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.vehicleDescriptionTeaser LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.engine LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.displacement LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.cylinders LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.compression LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.fuelManagement LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.fuel LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.horsePower LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.maxSpeed LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.weight LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.brakes LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.gearbox LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.rim LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.tire LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.interior LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.titleTextColor LIKE \'%' . $fragment . '%\'';
        } else {
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.workflowState = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numVehicles';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('RK\ParkHausModule\Entity\VehicleEntity', 'tbl');
     ...
     * @param int    $excludeId  Id of vehicles to exclude (optional)
     *
     * @return boolean result of this check, true if the given vehicle does not already exist
     */
    ...
    {
        $qb->leftJoin('tbl.vehicleImages', 'tblVehicleImages');
