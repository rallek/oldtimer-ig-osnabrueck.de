 ...
 */
abstract class AbstractVehicleController extends AbstractController
{
    /**
    ...
    {
        return $this->displayInternal($request, $vehicle, true);
    }
    ...
    {
        return $this->displayInternal($request, $vehicle, false);
    }
    
    /**
     * This method includes the common implementation code for adminDisplay() and display().
     */
    protected function displayInternal(Request $request, VehicleEntity $vehicle, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'vehicle';
        $permLevel = $isAdmin ? ACCESS_ADMIN : ACCESS_READ;
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemid) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'RKParkHausModule', 'user' => $userName, 'action' => $action, 'entity' => 'vehicle', 'id' => $itemid, 'errorMessage' => $e->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                ...
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'RKParkHausModule', 'user' => $userName, 'entity' => 'vehicle', 'id' => $itemid]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'RKParkHausModule', 'user' => $userName, 'action' => $action, 'entity' => 'vehicle', 'id' => $itemid]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('rkparkhausmodule_vehicle_' . /*($isAdmin ? 'admin' : '') . */'display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('rkparkhausmodule_vehicle_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
