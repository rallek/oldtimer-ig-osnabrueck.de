...
abstract class AbstractPersonEntity extends EntityAccess
{
     ...
     *
     * @param integer $registeredUser
     *
     * @return void
     */
    public function setRegisteredUser($registeredUser)
    {
        $this->registeredUser = intval($registeredUser);
    }
    ...
    {
        $serviceManager = ServiceUtil::getManager();
        $helper = $serviceManager->get('rk_team_module.listentries_helper');
    ...
    public function initWorkflow($forceLoading = false)
    {
        ...
        if ((!in_array($currentFunc, ['index', 'view', 'display']) && empty($isReuse)) || $forceLoading) {
            $result = Zikula_Workflow_Util::getWorkflowForObject($this, $this['_objectType'], $idColumn, 'RKTeamModule');
            if (!$result) {
    ...
    public function validate()
    {
        ...
        if (false !== strpos($this['personEmailAddress'], '&#')) {
            $this['personEmailAddress'] = html_entity_decode($this['personEmailAddress']);
        }
        $serviceManager = ServiceUtil::getManager();
    ...
    {
        return 'Person ' . $this->createCompositeIdentifier();
    }
