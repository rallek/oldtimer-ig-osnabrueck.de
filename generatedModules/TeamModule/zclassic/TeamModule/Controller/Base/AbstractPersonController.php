...
abstract class AbstractPersonController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        $objectType = 'person';
        $utilArgs = ['controller' => 'person', 'action' => 'index'];
        ...
        if ($isAdmin) {
            ...
            return $this->redirectToRoute('rkteammodule_person_' . ($isAdmin ? 'admin' : '') . 'view');
        }
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'person';
        $utilArgs = ['controller' => 'person', 'action' => 'view'];
        $permLevel = $isAdmin ? ACCESS_ADMIN : ACCESS_READ;
        if (!$this->hasPermission($this->name . ':' . ucfirst($objectType) . ':', '::', $permLevel)) {
            throw new AccessDeniedException();
        }
        $repository = $this->get('rk_team_module.' . $objectType . '_factory')->getRepository();
        ...
        $quickNavForm = $this->createForm('RK\TeamModule\Form\Type\QuickNavigation\\' . ucfirst($objectType) . 'QuickNavType', $templateParameters);
        ...
        $currentUrlObject = new RouteUrl('rkteammodule_person_' . /*($isAdmin ? 'admin' : '') . */'view', $currentUrlArgs);
        ...
        $modelHelper = $this->get('rk_team_module.model_helper');
    ...
    protected function displayInternal(Request $request, PersonEntity $person, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'person';
        $utilArgs = ['controller' => 'person', 'action' => 'display'];
        $permLevel = $isAdmin ? ACCESS_ADMIN : ACCESS_READ;
        if (!$this->hasPermission($this->name . ':' . ucfirst($objectType) . ':', '::', $permLevel)) {
            throw new AccessDeniedException();
        }
        ...
        $currentUrlArgs['_locale'] = $request->getLocale();
        $currentUrlObject = new RouteUrl('rkteammodule_person_' . /*($isAdmin ? 'admin' : '') . */'display', $currentUrlArgs);
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'person';
        $utilArgs = ['controller' => 'person', 'action' => 'edit'];
    ...
    protected function deleteInternal(Request $request, PersonEntity $person, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'person';
        $utilArgs = ['controller' => 'person', 'action' => 'delete'];
        ...
        $logArgs = ['app' => 'RKTeamModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'person', 'id' => $entity->createCompositeIdentifier()];
        ...
        $form = $this->createForm('RK\TeamModule\Form\DeleteEntityType', $entity);
