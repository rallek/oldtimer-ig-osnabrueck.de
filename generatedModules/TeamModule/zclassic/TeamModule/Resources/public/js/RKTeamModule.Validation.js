 ...
 */
function rKTeamPerformCustomValidationRules(objectType, currentEntityId)
{
    jQuery('.validate-nospace').each( function() {
        if (!rKTeamValidateNoSpace(jQuery(this).val())) {
            document.getElementById(jQuery(this).attr('id')).setCustomValidity(/*Zikula.__(*/'This value must not contain spaces.'/*, 'rkteammodule_js')*/);
        } else {
    ...
    jQuery('.validate-upload').each( function() {
        if (!rKTeamValidateUploadExtension(jQuery(this).val(), jQuery(this))) {
            document.getElementById(jQuery(this).attr('id')).setCustomValidity(/*Zikula.__(*/'Please select a valid file extension.'/*, 'rkteammodule_js')*/);
        } else {
            document.getElementById(jQuery(this).attr('id')).setCustomValidity('');
        }
    });
