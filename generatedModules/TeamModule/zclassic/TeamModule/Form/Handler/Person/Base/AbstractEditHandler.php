...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    public function processForm(array $templateParameters)
    {
        ...
        if ($this->templateParameters['mode'] == 'create') {
            $modelHelper = $this->container->get('rk_team_module.model_helper');
            if (!$modelHelper->canBeCreated($this->objectType)) {
                ...
                $logArgs = ['app' => 'RKTeamModule', 'user' => $this->container->get('zikula_users_module.current_user')->get('uname'), 'entity' => $this->objectType];
    ...
    protected function createForm()
    {
        ...
        return $this->container->get('form.factory')->create('RK\TeamModule\Form\Type\PersonType', $this->entityRef, $options);
    }
    ...
    protected function getDefaultReturnUrl($args)
    {
        ...
        $viewArgs = [];
        $url = $this->router->generate('rkteammodule_' . $this->objectTypeLower . '_' . $routeArea . 'view', $viewArgs);
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'RKTeamModule', 'user' => $this->container->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'person', 'id' => $entity->createCompositeIdentifier(), 'errorMessage' => $e->getMessage()];
    ...
    protected function getRedirectUrl($args)
    {
        ...
        switch ($this->returnTo) {
            case 'admin':
                return $this->router->generate('rkteammodule_' . $this->objectTypeLower . '_adminindex');
            case 'adminView':
                return $this->router->generate('rkteammodule_' . $this->objectTypeLower . '_adminview');
            case 'adminDisplay':
                if ($args['commandName'] != 'delete' && !($this->templateParameters['mode'] == 'create' && $args['commandName'] == 'cancel')) {
                    ...
                    return $this->router->generate('rkteammodule_' . $this->objectTypeLower . '_admindisplay', $urlArgs);
                }
                return $this->getDefaultReturnUrl($args);
            case 'user':
                return $this->router->generate('rkteammodule_' . $this->objectTypeLower . '_index');
            case 'userView':
                return $this->router->generate('rkteammodule_' . $this->objectTypeLower . '_view');
            case 'userDisplay':
                if ($args['commandName'] != 'delete' && !($this->templateParameters['mode'] == 'create' && $args['commandName'] == 'cancel')) {
                    foreach ($this->idFields as $idField) {
                        $urlArgs[$idField] = $this->idValues[$idField];
                    }
                    return $this->router->generate('rkteammodule_' . $this->objectTypeLower . '_display', $urlArgs);
                }
                return $this->getDefaultReturnUrl($args);
            default:
                return $this->getDefaultReturnUrl($args);
