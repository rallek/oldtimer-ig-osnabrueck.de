...
abstract class AbstractUploadHandler
{
    ...
    * @param VariableApi         $variableApi    VariableApi service instance
    ...
    public function performFileUpload($objectType, $file, $fieldName)
    {
        ...
        $controllerHelper = $serviceManager->get('rk_team_module.controller_helper');
        ...
        try {
            ...
            $logger->error('{app}: User {user} could not detect upload destination path for entity {entity} and field {field}.', ['app' => 'RKTeamModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $objectType, 'field' => $fieldName]);
        ...
        if ($isImage) {
            ...
            if (!is_array($imgInfo) || !$imgInfo[0] || !$imgInfo[1]) {
                ...
                $logger->error('{app}: User {user} tried to upload a file which is seems not to be a valid image.', ['app' => 'RKTeamModule', 'user' => $this->currentUserApi->get('uname')]);
    ...
    protected function validateFileUpload($objectType, $file, $fieldName)
    {
        ...
        if ($file->getError() != UPLOAD_ERR_OK) {
            ...
            $logger->error('{app}: User {user} tried to upload a file with errors: ' . $file->getErrorMessage(), ['app' => 'RKTeamModule', 'user' => $this->currentUserApi->get('uname')]);
        ...
        if (false === $isValidExtension) {
            ...
            $logger->error('{app}: User {user} tried to upload a file with a forbidden extension ("{extension}").', ['app' => 'RKTeamModule', 'user' => $this->currentUserApi->get('uname'), 'extension' => $extension]);
