...
namespace RK\SponsoringModule\Entity\Repository\Base;
...
use RK\SponsoringModule\Helper\HookHelper;
use RK\SponsoringModule\Helper\ImageHelper;
use RK\SponsoringModule\Helper\WorkflowHelper;

/**
 * Repository class used to implement own convenience methods for performing certain DQL queries.
 *
 * This is the base repository class for sponsor entities.
 */
abstract class AbstractSponsorRepository extends EntityRepository
{
    /**
    ...
    public function getAdditionalTemplateParameters(ImageHelper $imageHelper, $context = '', $args = [])
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], ['display', 'view'])) {
                ...
                $templateParameters['relationThumbRuntimeOptions'] = $imageHelper->getCustomRuntimeOptions('', '', 'RKSponsoringModule_relateditem', $context, $args);
            }
        }
    ...
    protected function getViewQuickNavParameters($context = '', $args = [])
    {
        ...
        $parameters['sponsoringUser'] = (int) $this->getRequest()->query->get('sponsoringUser', 0);
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('RK\SponsoringModule\Entity\SponsorEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'RKSponsoringModule', 'entity' => 'sponsor'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('RK\SponsoringModule\Entity\SponsorEntity', 'tbl')
        ...
        $logArgs = ['app' => 'RKSponsoringModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'sponsors', 'userid' => $userId];
    ...
    public function updateUserField($userFieldName, $userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        if (empty($userFieldName) || !in_array($userFieldName, ['sponsoringUser'])) {
        ...
        $logArgs = ['app' => 'RKSponsoringModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'sponsors', 'field' => $userFieldName, 'userid' => $userId, 'newuserid' => $newUserId];
    ...
    public function deleteByUserField($userFieldName, $userId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $logArgs = ['app' => 'RKSponsoringModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'sponsors', 'field' => $userFieldName, 'userid' => $userId];
     ...
     *
     * @return array|sponsorEntity retrieved data array or sponsorEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection collection containing retrieved sponsorEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = [])
    {
        ...
        $isAdminArea = false !== strpos($routeName, 'rksponsoringmodule_sponsor_admin');
        if ($isAdminArea) {
            return $qb;
        }
    
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved sponsors only
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.sponsorName LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.sponsoringEvent LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            ...
            $where .= 'tbl.endDate = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.sponsoringUser = \'' . $fragment . '\'';
        }
        $where = '(' . $where . ')';
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numSponsors';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('RK\SponsoringModule\Entity\SponsorEntity', 'tbl');
     ...
     * @param int    $excludeId  Id of sponsors to exclude (optional)
     *
     * @return boolean result of this check, true if the given sponsor does not already exist
     */
    ...
    public function genericBaseQuery($where = '', $orderBy = '', $useJoins = true, $slimMode = false)
    {
        ...
        if (true === $slimMode) {
            ...
            $selection .= ', tbl.sponsorName';
            
            
            $selection .= ', tbl.sponsoringEvent';
            $useJoins = false;
        }
    ...
    /**
     * Update for sponsors becoming archived.
     *
    ...
    {
        if (true !== \PageUtil::getVar('RKSponsoringModuleAutomaticArchiving', false) && !$permissionApi->hasPermission('RKSponsoringModule', '.*', ACCESS_EDIT)) {
        ...
        foreach ($affectedEntities as $entity) {
            ...
            $url = new RouteUrl('rksponsoringmodule_sponsor_display', $urlArgs);
