...
abstract class AbstractControllerHelper
{
    ...
    /**
     * Returns an array of all allowed object types in RKSponsoringModule.
     *
    ...
    public function getObjectTypes($context = '', $args = [])
    {
        ...
        $allowedObjectTypes = [];
        $allowedObjectTypes[] = 'sponsor';
    
        return $allowedObjectTypes;
    }

    /**
     * Returns the default object type in RKSponsoringModule.
     *
     * @param string $context Usage context (allowed values: controllerAction, api, helper, actionHandler, block, contentType, util)
     * @param array  $args    Additional arguments
     *
    ...
    public function getFileBaseFolder($objectType, $fieldName, $ignoreCreate = false)
    {
        ...
        $basePath = $this->container->getParameter('datadir') . '/RKSponsoringModule/';
    
        switch ($objectType) {
            case 'sponsor':
                $basePath .= 'sponsors/logo/';
            break;
        }
    ...
    public function checkAndCreateAllUploadFolders()
    {
        ...
        $result &= $this->checkAndCreateUploadFolder('sponsor', 'logo', 'gif, jpeg, jpg, png');
    ...
    protected function checkAndCreateUploadFolder($objectType, $fieldName, $allowedExtensions = '')
    {
        ...
        if (!$fs->exists($uploadPath)) {
            try {
                ...
                $this->logger->error('{app}: The upload directory {directory} does not exist and could not be created.', ['app' => 'RKSponsoringModule', 'directory' => $uploadPath]);
        ...
        if (!is_writable($uploadPath)) {
            try {
                ...
                $this->logger->error('{app}: The upload directory {directory} exists but is not writable by the webserver.', ['app' => 'RKSponsoringModule', 'directory' => $uploadPath]);
        ...
        $htaccessFileTemplate = 'modules/RKSponsoringModule/Resources/docs/htaccessTemplate';
        if (!$fs->exists($htaccessFilePath) && $fs->exists($htaccessFileTemplate)) {
            try {
                ...
                $this->logger->error('{app}: An error occured during creation of the .htaccess file in directory {directory}.', ['app' => 'RKSponsoringModule', 'directory' => $uploadPath]);
            }
        }
