...
/**
 * Sponsor controller base class.
 */
abstract class AbstractSponsorController extends AbstractController
{
    /**
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        $objectType = 'sponsor';
        $utilArgs = ['controller' => 'sponsor', 'action' => 'index'];
        ...
        if ($isAdmin) {
            ...
            return $this->redirectToRoute('rksponsoringmodule_sponsor_' . ($isAdmin ? 'admin' : '') . 'view');
        }
        ...
        return $this->render('@RKSponsoringModule/Sponsor/index.html.twig', $templateParameters);
    }
    /**
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'sponsor';
        $utilArgs = ['controller' => 'sponsor', 'action' => 'view'];
        ...
        $viewHelper = $this->get('rk_sponsoring_module.view_helper');
        $templateParameters = [
            'routeArea' => $isAdmin ? 'admin' : ''
        ];
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'rksponsoringmodule_sponsor_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
        $sortableColumns->addColumns([
            new Column('workflowState'),
            new Column('sponsorName'),
            new Column('sponsoringEvent'),
            new Column('logo'),
            new Column('description'),
            new Column('startDate'),
            new Column('endDate'),
            new Column('sponsoringUser'),
        ...
        $quickNavForm = $this->createForm('RK\SponsoringModule\Form\Type\QuickNavigation\\' . ucfirst($objectType) . 'QuickNavType', $templateParameters);
        ...
        $currentUrlObject = new RouteUrl('rksponsoringmodule_sponsor_' . /*($isAdmin ? 'admin' : '') . */'view', $currentUrlArgs);
        ...
        $modelHelper = $this->get('rk_sponsoring_module.model_helper');
     ...
     * @ParamConverter("sponsor", class="RKSponsoringModule:SponsorEntity", options={"id" = "id", "repository_method" = "selectById"})
     * @Cache(lastModified="sponsor.getUpdatedDate()", ETag="'Sponsor' ~ sponsor.getid() ~ sponsor.getUpdatedDate().format('U')")
     *
     * @param Request  $request      Current request instance
    ...
    {
        return $this->displayInternal($request, $sponsor, true);
    }
    ...
    {
        return $this->displayInternal($request, $sponsor, false);
    }
    
    /**
     * This method includes the common implementation code for adminDisplay() and display().
     */
    protected function displayInternal(Request $request, SponsorEntity $sponsor, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'sponsor';
        $utilArgs = ['controller' => 'sponsor', 'action' => 'display'];
        $permLevel = $isAdmin ? ACCESS_ADMIN : ACCESS_READ;
        if (!$this->hasPermission($this->name . ':' . ucfirst($objectType) . ':', '::', $permLevel)) {
            throw new AccessDeniedException();
        }
        ...
        $currentUrlArgs['_locale'] = $request->getLocale();
        $currentUrlObject = new RouteUrl('rksponsoringmodule_sponsor_' . /*($isAdmin ? 'admin' : '') . */'display', $currentUrlArgs);
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'sponsor';
        $utilArgs = ['controller' => 'sponsor', 'action' => 'edit'];
        ...
        $formHandler = $this->get('rk_sponsoring_module.form.handler.sponsor');
    ...
    {
        return $this->deleteInternal($request, $sponsor, true);
    }
    ...
    {
        return $this->deleteInternal($request, $sponsor, false);
    }
    
    /**
     * This method includes the common implementation code for adminDelete() and delete().
     */
    protected function deleteInternal(Request $request, SponsorEntity $sponsor, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'sponsor';
        $utilArgs = ['controller' => 'sponsor', 'action' => 'delete'];
        ...
        $logArgs = ['app' => 'RKSponsoringModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'sponsor', 'id' => $entity->createCompositeIdentifier()];
        
        $entity->initWorkflow();
        
        // determine available workflow actions
        $workflowHelper = $this->get('rk_sponsoring_module.workflow_helper');
        ...
        if ($isAdmin) {
            // redirect to the list of sponsors
            $redirectRoute = 'rksponsoringmodule_sponsor_' . ($isAdmin ? 'admin' : '') . 'view';
        } else {
            // redirect to the list of sponsors
            $redirectRoute = 'rksponsoringmodule_sponsor_' . ($isAdmin ? 'admin' : '') . 'view';
        }
        ...
        if (!$deleteAllowed) {
            $this->addFlash('error', $this->__('Error! It is not allowed to delete this sponsor.'));
            $logger->error('{app}: User {user} tried to delete the {entity} with id {id}, but this action was not allowed.', $logArgs);
        
            return $this->redirectToRoute($redirectRoute);
        }
        
        $form = $this->createForm('RK\SponsoringModule\Form\DeleteEntityType', $entity);
        
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                $hookHelper = $this->get('rk_sponsoring_module.hook_helper');
                ...
                if ($validationHooksPassed) {
                    ...
                    // Let any hooks know that we have deleted the sponsor
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemid) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'RKSponsoringModule', 'user' => $userName, 'action' => $action, 'entity' => 'sponsor', 'id' => $itemid, 'errorMessage' => $e->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'RKSponsoringModule', 'user' => $userName, 'entity' => 'sponsor', 'id' => $itemid]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'RKSponsoringModule', 'user' => $userName, 'action' => $action, 'entity' => 'sponsor', 'id' => $itemid]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('rksponsoringmodule_sponsor_' . /*($isAdmin ? 'admin' : '') . */'display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('rksponsoringmodule_sponsor_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
