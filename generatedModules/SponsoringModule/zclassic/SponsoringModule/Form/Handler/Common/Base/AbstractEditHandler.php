...
namespace RK\SponsoringModule\Form\Handler\Common\Base;
...
abstract class AbstractEditHandler
{
    ...
    public function processForm(array $templateParameters)
    {
        ...
        $this->permissionComponent = 'RKSponsoringModule:' . $this->objectTypeCapital . ':';
    
        $selectionHelper = $this->container->get('rk_sponsoring_module.selection_helper');
        ...
        if ($this->templateParameters['mode'] == 'edit') {
            ...
            if (true === $this->hasPageLockSupport && \ModUtil::available('ZikulaPageLockModule')) {
                ...
                $lockName = 'RKSponsoringModule' . $this->objectTypeCapital . $this->createCompositeIdentifier();
        ...
        if (false === $actions || !is_array($actions)) {
            ...
            $logArgs = ['app' => 'RKSponsoringModule', 'user' => $this->container->get('zikula_users_module.current_user')->get('uname'), 'entity' => $this->objectType, 'id' => $entity->createCompositeIdentifier()];
    ...
    protected function initEntityForCreation()
    {
        ...
        if (is_null($entity)) {
            $factory = $this->container->get('rk_sponsoring_module.' . $this->objectType . '_factory');
    ...
    public function handleCommand($args = [])
    {
        ...
        if ($entity->supportsHookSubscribers() && $action != 'cancel') {
            $hookHelper = $this->container->get('rk_sponsoring_module.hook_helper');
        ...
        if ($isRegularAction || $action == 'delete') {
            ...
            if ($entity->supportsHookSubscribers()) {
                ...
                if ($action != 'delete') {
                    ...
                    $url = new RouteUrl('rksponsoringmodule_' . $this->objectType . '_display', $urlArgs);
                }
    ...
    protected function addDefaultMessage($args, $success = false)
    {
        ...
        $logger = $this->container->get('logger');
        $logArgs = ['app' => 'RKSponsoringModule', 'user' => $this->container->get('zikula_users_module.current_user')->get('uname'), 'entity' => $this->objectType, 'id' => $this->entityRef->createCompositeIdentifier()];
        if (true === $success) {
    ...
    public function fetchInputData($args)
    {
        ...
        if (isset($this->form['additionalNotificationRemarks']) && $this->form['additionalNotificationRemarks']->getData() != '') {
            $this->request->getSession()->set('RKSponsoringModuleAdditionalNotificationRemarks', $this->form['additionalNotificationRemarks']->getData());
        }
