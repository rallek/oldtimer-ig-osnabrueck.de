...
abstract class AbstractUploadHandler
{
    ...
    * @param VariableApi         $variableApi    VariableApi service instance
    ...
    public function __construct(TranslatorInterface $translator, CurrentUserApi $currentUserApi, VariableApi $variableApi)
    {
        ...
        $this->allowedObjectTypes = ['sponsor'];
    ...
    public function performFileUpload($objectType, $file, $fieldName)
    {
        ...
        try {
            ...
            $logger->error('{app}: User {user} could not detect upload destination path for entity {entity} and field {field}.', ['app' => 'RKSponsoringModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $objectType, 'field' => $fieldName]);
        ...
        if ($isImage) {
            ...
            if (!is_array($imgInfo) || !$imgInfo[0] || !$imgInfo[1]) {
                ...
                $logger->error('{app}: User {user} tried to upload a file which is seems not to be a valid image.', ['app' => 'RKSponsoringModule', 'user' => $this->currentUserApi->get('uname')]);
        ...
        if ($isImage) {
            ...
            if (true === $this->variableApi->get('RKSponsoringModule', 'enableShrinkingFor' . $fieldSuffix, false)) {
                // check for maximum size
                $maxWidth = $this->variableApi->get('RKSponsoringModule', 'shrinkWidth' . $fieldSuffix, 800);
                $maxHeight = $this->variableApi->get('RKSponsoringModule', 'shrinkHeight' . $fieldSuffix, 600);
                ...
                if ($imgInfo[0] > $maxWidth || $imgInfo[1] > $maxHeight) {
                    ...
                    $preset = new \SystemPlugin_Imagine_Preset('RKSponsoringModule_Shrinker', [
                        'width' => $maxWidth,
                        'height' => $maxHeight,
                        'mode' => 'inset'
                    ]);
    ...
    protected function validateFileUpload($objectType, $file, $fieldName)
    {
        ...
        if ($file->getError() != UPLOAD_ERR_OK) {
            ...
            $logger->error('{app}: User {user} tried to upload a file with errors: ' . $file->getErrorMessage(), ['app' => 'RKSponsoringModule', 'user' => $this->currentUserApi->get('uname')]);
        ...
        if (false === $isValidExtension) {
            ...
            $logger->error('{app}: User {user} tried to upload a file with a forbidden extension ("{extension}").', ['app' => 'RKSponsoringModule', 'user' => $this->currentUserApi->get('uname'), 'extension' => $extension]);
