...
abstract class AbstractSearchHelper extends AbstractSearchable
{
    ...
    public function getOptions($active, $modVars = null)
    {
        ...
        $searchTypes = array('sponsor');
        foreach ($searchTypes as $searchType) {
            $templateParameters['active_' . $searchType] = (!isset($args['rKSponsoringModuleSearchTypes']) || in_array($searchType, $args['rKSponsoringModuleSearchTypes']));
        }
    
        return $this->getContainer()->get('twig')->render('@RKSponsoringModule/Search/options.html.twig', $templateParameters);
    }
    ...
    public function getResults(array $words, $searchType = 'AND', $modVars = null)
    {
        ...
        if (!is_array($searchTypes) || !count($searchTypes)) {
            if ($request->isMethod('GET')) {
                $searchTypes = $request->query->get('rKSponsoringModuleSearchTypes', []);
            } elseif ($request->isMethod('POST')) {
                $searchTypes = $request->request->get('rKSponsoringModuleSearchTypes', []);
            }
        }
        ...
        foreach ($searchTypes as $objectType) {
            ...
            switch ($objectType) {
                case 'sponsor':
                    ...
                    $whereArray[] = 'tbl.sponsorName';
                    $whereArray[] = 'tbl.sponsoringEvent';
                    $whereArray[] = 'tbl.logo';
                    $whereArray[] = 'tbl.description';
                    break;
            }
    
            $repository = $serviceManager->get('rk_sponsoring_module.' . $objectType . '_factory')->getRepository();
            ...
            $entitiesWithDisplayAction = ['sponsor'];
            ...
            foreach ($entities as $entity) {
                ...
                $displayUrl = $hasDisplayAction ? new RouteUrl('rksponsoringmodule_' . $objectType . '_display', $urlArgs) : '';
