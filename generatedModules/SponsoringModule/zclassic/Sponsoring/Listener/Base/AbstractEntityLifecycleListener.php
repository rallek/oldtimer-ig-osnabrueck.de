...
use RK\SponsoringModule\SponsoringEvents;
use RK\SponsoringModule\Event\FilterSponsorEvent;
...
abstract class AbstractEntityLifecycleListener implements EventSubscriber
{
    ...
    public function preRemove(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\RK\\SponsoringModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $dispatcher->dispatch(constant('\\RK\\SponsoringModule\\SponsoringEvents::' . strtoupper($entity->get_objectType()) . '_PRE_REMOVE'), $event);
    ...
    public function postRemove(LifecycleEventArgs $args)
    {
        ...
        $uploadManager = $serviceManager->get('rk_sponsoring_module.upload_handler');
        ...
        $logArgs = ['app' => 'RKSponsoringModule', 'user' => $serviceManager->get('zikula_users_module.current_user')->get('uname'), 'entity' => $objectType, 'id' => $objectId];
        $logger->debug('{app}: User {user} removed the {entity} with id {id}.', $logArgs);
        
        $dispatcher = $serviceManager->get('event_dispatcher');
        
        // create the filter event and dispatch it
        $filterEventClass = '\\RK\\SponsoringModule\\Event\\Filter' . ucfirst($objectType) . 'Event';
        $event = new $filterEventClass($entity);
        $dispatcher->dispatch(constant('\\RK\\SponsoringModule\\SponsoringEvents::' . strtoupper($objectType) . '_POST_REMOVE'), $event);
    }
    ...
    public function prePersist(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\RK\\SponsoringModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $dispatcher->dispatch(constant('\\RK\\SponsoringModule\\SponsoringEvents::' . strtoupper($entity->get_objectType()) . '_PRE_PERSIST'), $event);
        if ($event->isPropagationStopped()) {
            return false;
        }
    }
    ...
    public function postPersist(LifecycleEventArgs $args)
    {
        ...
        $objectId = $entity->createCompositeIdentifier();
        $logger = $serviceManager->get('logger');
        $logArgs = ['app' => 'RKSponsoringModule', 'user' => $serviceManager->get('zikula_users_module.current_user')->get('uname'), 'entity' => $entity->get_objectType(), 'id' => $objectId];
        ...
        $filterEventClass = '\\RK\\SponsoringModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $dispatcher->dispatch(constant('\\RK\\SponsoringModule\\SponsoringEvents::' . strtoupper($entity->get_objectType()) . '_POST_PERSIST'), $event);
    }
    ...
    public function preUpdate(PreUpdateEventArgs $args)
    {
        ...
        $filterEventClass = '\\RK\\SponsoringModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $dispatcher->dispatch(constant('\\RK\\SponsoringModule\\SponsoringEvents::' . strtoupper($entity->get_objectType()) . '_PRE_UPDATE'), $event);
        if ($event->isPropagationStopped()) {
            return false;
        }
    }
    ...
    public function postUpdate(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\RK\\SponsoringModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $dispatcher->dispatch(constant('\\RK\\SponsoringModule\\SponsoringEvents::' . strtoupper($entity->get_objectType()) . '_POST_UPDATE'), $event);
    }
    ...
    public function postLoad(LifecycleEventArgs $args)
    {
        ...
        $objectType = $entity->get_objectType();
        $controllerHelper = ServiceUtil::get('rk_sponsoring_module.controller_helper');
        ...
        $filterEventClass = '\\RK\\SponsoringModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $dispatcher->dispatch(constant('\\RK\\SponsoringModule\\SponsoringEvents::' . strtoupper($entity->get_objectType()) . '_POST_LOAD'), $event);
    }
    ...
    protected function isEntityManagedByThisBundle($entity)
    {
        ...
        return ($entityClassParts[0] == 'RK' && $entityClassParts[1] == 'SponsoringModule');
    }
