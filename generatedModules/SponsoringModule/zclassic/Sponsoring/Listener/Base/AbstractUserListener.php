...
abstract class AbstractUserListener implements EventSubscriberInterface
{
    ...
    public function delete(GenericEvent $event)
    {
        ...
        $repo = $entityManager->getRepository('RK\SponsoringModule\Entity\SponsorEntity');
        // set creator to admin (2) for all sponsors created by this user
        $repo->updateCreator($uid, 2, $translator, $logger, $currentUserApi);
        
        // set last editor to admin (2) for all sponsors updated by this user
        $repo->updateLastEditor($uid, 2, $translator, $logger, $currentUserApi);
        // set last editor to guest (1) for all sponsors affected by this user
        $repo->updateUserField('sponsoringUser', $uid, 1, $translator, $logger, $currentUserApi);
        
        $logArgs = ['app' => 'RKSponsoringModule', 'user' => $serviceManager->get('zikula_users_module.current_user')->get('uname'), 'entities' => 'sponsors'];
