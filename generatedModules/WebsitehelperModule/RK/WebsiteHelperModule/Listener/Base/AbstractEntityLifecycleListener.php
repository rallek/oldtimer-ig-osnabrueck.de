...
use RK\WebsiteHelperModule\WebsiteHelperEvents;
use RK\WebsiteHelperModule\Event\FilterLinkerEvent;
use RK\WebsiteHelperModule\Event\FilterCarouselItemEvent;
use RK\WebsiteHelperModule\Event\FilterCarouselEvent;
use RK\WebsiteHelperModule\Event\FilterWebsiteImageEvent;
...
abstract class AbstractEntityLifecycleListener implements EventSubscriber, ContainerAwareInterface
{
    ...
    public function preRemove(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\RK\\WebsiteHelperModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\WebsiteHelperModule\\WebsiteHelperEvents::' . strtoupper($entity->get_objectType()) . '_PRE_REMOVE'), $event);
    ...
    public function postRemove(LifecycleEventArgs $args)
    {
        ...
        $uploadHelper = $this->container->get('rk_websitehelper_module.upload_helper');
        ...
        $logArgs = ['app' => 'RKWebsiteHelperModule', 'user' => $this->container->get('zikula_users_module.current_user')->get('uname'), 'entity' => $objectType, 'id' => $objectId];
        $logger->debug('{app}: User {user} removed the {entity} with id {id}.', $logArgs);
        
        // create the filter event and dispatch it
        $filterEventClass = '\\RK\\WebsiteHelperModule\\Event\\Filter' . ucfirst($objectType) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\WebsiteHelperModule\\WebsiteHelperEvents::' . strtoupper($objectType) . '_POST_REMOVE'), $event);
    }
    ...
    public function prePersist(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\RK\\WebsiteHelperModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\WebsiteHelperModule\\WebsiteHelperEvents::' . strtoupper($entity->get_objectType()) . '_PRE_PERSIST'), $event);
        if ($event->isPropagationStopped()) {
            return false;
        }
    }
    ...
    public function postPersist(LifecycleEventArgs $args)
    {
        ...
        $objectId = $entity->createCompositeIdentifier();
        $logger = $this->container->get('logger');
        $logArgs = ['app' => 'RKWebsiteHelperModule', 'user' => $this->container->get('zikula_users_module.current_user')->get('uname'), 'entity' => $entity->get_objectType(), 'id' => $objectId];
        $logger->debug('{app}: User {user} created the {entity} with id {id}.', $logArgs);
        
        // create the filter event and dispatch it
        $filterEventClass = '\\RK\\WebsiteHelperModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\WebsiteHelperModule\\WebsiteHelperEvents::' . strtoupper($entity->get_objectType()) . '_POST_PERSIST'), $event);
    }
    ...
    public function preUpdate(PreUpdateEventArgs $args)
    {
        ...
        $filterEventClass = '\\RK\\WebsiteHelperModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\WebsiteHelperModule\\WebsiteHelperEvents::' . strtoupper($entity->get_objectType()) . '_PRE_UPDATE'), $event);
        if ($event->isPropagationStopped()) {
            return false;
        }
    }
    ...
    public function postUpdate(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\RK\\WebsiteHelperModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\WebsiteHelperModule\\WebsiteHelperEvents::' . strtoupper($entity->get_objectType()) . '_POST_UPDATE'), $event);
    }
    ...
    public function postLoad(LifecycleEventArgs $args)
    {
        ...
        $filterEventClass = '\\RK\\WebsiteHelperModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
        $event = new $filterEventClass($entity);
        $this->container->get('event_dispatcher')->dispatch(constant('\\RK\\WebsiteHelperModule\\WebsiteHelperEvents::' . strtoupper($entity->get_objectType()) . '_POST_LOAD'), $event);
    }
    ...
    protected function isEntityManagedByThisBundle($entity)
    {
        ...
        return ($entityClassParts[0] == 'RK' && $entityClassParts[1] == 'WebsiteHelperModule');
    }
