 ...
 *
 * This is the base repository class for website image entities.
 */
abstract class AbstractWebsiteImageRepository extends EntityRepository
{
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('RK\WebsiteHelperModule\Entity\WebsiteImageEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'RKWebsiteHelperModule', 'entity' => 'website image'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('RK\WebsiteHelperModule\Entity\WebsiteImageEntity', 'tbl')
        ...
        $logArgs = ['app' => 'RKWebsiteHelperModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'website images', 'userid' => $userId];
     ...
     *
     * @return array|websiteImageEntity retrieved data array or websiteImageEntity instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection collection containing retrieved websiteImageEntity instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $selection = 'COUNT(tbl.id) AS numWebsiteImages';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('RK\WebsiteHelperModule\Entity\WebsiteImageEntity', 'tbl');
     ...
     * @param int    $excludeId  Id of website images to exclude (optional)
     *
     * @return boolean result of this check, true if the given website image does not already exist
     */
