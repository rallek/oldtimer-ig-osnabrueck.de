...
abstract class AbstractLinkerController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('rkwebsitehelpermodule_linker_' . $templateParameters['routeArea'] . 'view');
        
        // return index template
        return $this->render('@RKWebsiteHelperModule/Linker/index.html.twig', $templateParameters);
    }
    /**
     ...
     * @ParamConverter("linker", class="RKWebsiteHelperModule:LinkerEntity", options={"id" = "id", "repository_method" = "selectById"})
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('rk_websitehelper_module.form.handler.linker');
    ...
    protected function deleteInternal(Request $request, LinkerEntity $linker, $isAdmin = false)
    {
        ...
        $logArgs = ['app' => 'RKWebsiteHelperModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'linker', 'id' => $linker->createCompositeIdentifier()];
        ...
        $redirectRoute = 'rkwebsitehelpermodule_linker_' . ($isAdmin ? 'admin' : '') . 'view';
        ...
        $form = $this->createForm('RK\WebsiteHelperModule\Form\DeleteEntityType', $linker);
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemid) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'RKWebsiteHelperModule', 'user' => $userName, 'action' => $action, 'entity' => 'linker', 'id' => $itemid, 'errorMessage' => $e->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'RKWebsiteHelperModule', 'user' => $userName, 'entity' => 'linker', 'id' => $itemid]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'RKWebsiteHelperModule', 'user' => $userName, 'action' => $action, 'entity' => 'linker', 'id' => $itemid]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('rkwebsitehelpermodule_linker_' . /*($isAdmin ? 'admin' : '') . */'display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('rkwebsitehelpermodule_linker_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
