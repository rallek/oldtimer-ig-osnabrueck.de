...
namespace RK\WebsiteHelperModule\Form\Handler\Common\Base;
...
use RK\WebsiteHelperModule\Helper\HookHelper;
use RK\WebsiteHelperModule\Helper\ModelHelper;
use RK\WebsiteHelperModule\Helper\SelectionHelper;
...
abstract class AbstractEditHandler
{
     ...
     * @param WebsiteHelperFactory $entityFactory WebsiteHelperFactory service instance
    ...
    public function __construct(
        ...
        WebsiteHelperFactory $entityFactory,
    ...
    public function processForm(array $templateParameters)
    {
        ...
        if (null === $this->returnTo) {
            ...
            if ($this->request->getSession()->has('rkwebsitehelpermoduleReferer')) {
                $this->returnTo = $this->request->getSession()->get('rkwebsitehelpermoduleReferer');
            } elseif ($this->request->headers->has('rkwebsitehelpermoduleReferer')) {
                $this->returnTo = $this->request->headers->get('rkwebsitehelpermoduleReferer');
                $this->request->getSession()->set('rkwebsitehelpermoduleReferer', $this->returnTo);
        ...
        $this->permissionComponent = 'RKWebsiteHelperModule:' . $this->objectTypeCapital . ':';
        ...
        if ($this->templateParameters['mode'] == 'edit') {
            ...
            if (null !== $entity) {
                if (true === $this->hasPageLockSupport && $this->kernel->isBundle('ZikulaPageLockModule') && null !== $this->lockingApi) {
                    ...
                    $lockName = 'RKWebsiteHelperModule' . $this->objectTypeCapital . $this->createCompositeIdentifier();
        ...
        if (false === $actions || !is_array($actions)) {
            ...
            $logArgs = ['app' => 'RKWebsiteHelperModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $entity->createCompositeIdentifier()];
        ...
        if ($isRegularAction || $action == 'delete') {
            ...
            if ($entity->supportsHookSubscribers()) {
                ...
                if ($action != 'delete') {
                    ...
                    $url = new RouteUrl('rkwebsitehelpermodule_' . $this->objectType . '_display', $urlArgs);
                }
    ...
    protected function addDefaultMessage($args, $success = false)
    {
        ...
        $logArgs = ['app' => 'RKWebsiteHelperModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $this->entityRef->createCompositeIdentifier()];
        if (true === $success) {
    ...
    public function fetchInputData($args)
    {
        ...
        if (isset($this->form['additionalNotificationRemarks']) && $this->form['additionalNotificationRemarks']->getData() != '') {
            $this->request->getSession()->set('RKWebsiteHelperModuleAdditionalNotificationRemarks', $this->form['additionalNotificationRemarks']->getData());
        }
