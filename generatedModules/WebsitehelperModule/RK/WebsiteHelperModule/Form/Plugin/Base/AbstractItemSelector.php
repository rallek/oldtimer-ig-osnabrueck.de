...
namespace RK\WebsiteHelperModule\Form\Plugin\Base;
...
class AbstractItemSelector extends Zikula_Form_Plugin_TextInput implements ContainerAwareInterface
{
    ...
    public function render(Zikula_Form_View $view)
    {
        ...
        if ($firstTime) {
            ...
            $jsAssetBag->add($assetHelper->resolve('@RKWebsiteHelperModule:js/RKWebsiteHelperModule.Finder.js'));
            $cssAssetBag->add($assetHelper->resolve('@RKWebsiteHelperModule:css/style.css'));
        }
        $firstTime = false;

        $permissionApi = $this->container->get('zikula_permissions_module.api.permission');

        if (!$permissionApi->hasPermission('RKWebsiteHelperModule:' . ucfirst($this->objectType) . ':', '::', ACCESS_COMMENT)) {
            return false;
        }

        $this->selectedItemId = $this->text;

        $repository = $this->container->get('rk_websitehelper_module.entity_factory')->getRepository($this->objectType);
    ...
    {
        parent::decode($view);
        $this->objectType = $this->container->get('request_stack')->getCurrentRequest()->request->get('RKWebsiteHelperModule_objecttype', 'linker');
        $this->selectedItemId = $this->text;
    }
}
