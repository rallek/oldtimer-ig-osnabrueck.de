 ...
 */
function rKWebsiteHelperInitQuickNavigation()
{
    var quickNavForm;
    var objectType;

    if (jQuery('.rkwebsitehelpermodule-quicknav').length < 1) {
        return;
    }

    quickNavForm = jQuery('.rkwebsitehelpermodule-quicknav').first();
    objectType = quickNavForm.attr('id').replace('rKWebsiteHelperModule', '').replace('QuickNavForm', '');
 ...
 */
function rKWebsiteHelperInitMassToggle()
{
    if (jQuery('.rkwebsitehelper-mass-toggle').length > 0) {
        jQuery('.rkwebsitehelper-mass-toggle').click(function (event) {
            jQuery('.rkwebsitehelper-toggle-checkbox').prop('checked', jQuery(this).prop('checked'));
        });
    }
}
 ...
 */
function rKWebsiteHelperInitItemActions(context)
{
    ...
    containerSelector = '';
    if (context == 'view') {
        containerSelector = '.rkwebsitehelpermodule-view';
 ...
 */
function rKWebsiteHelperInitInlineWindow(containerElem)
{
    var newWindowId;
    var modalTitle;
 ...
 */
function rKWebsiteHelperInitQuickViewModals()
{
    jQuery('.rkwebsitehelper-inline-window').each(function (index) {
        rKWebsiteHelperInitInlineWindow(jQuery(this));
    });
}
...
jQuery(document).ready(function() {
    ...
    isViewPage = jQuery('.rkwebsitehelpermodule-view').length > 0;
    isDisplayPage = jQuery('.rkwebsitehelpermodule-display').length > 0;

    jQuery('a.lightbox').lightbox();

    if (isViewPage) {
        rKWebsiteHelperInitQuickNavigation();
        rKWebsiteHelperInitMassToggle();
        rKWebsiteHelperInitItemActions('view');
    } else if (isDisplayPage) {
        rKWebsiteHelperInitItemActions('display');
    }

    rKWebsiteHelperInitQuickViewModals();
});
