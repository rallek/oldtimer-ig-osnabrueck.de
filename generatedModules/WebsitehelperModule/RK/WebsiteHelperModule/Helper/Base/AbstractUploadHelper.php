...
namespace RK\WebsiteHelperModule\Helper\Base;
...
abstract class AbstractUploadHelper
{
        ...
        $dataDirectory)
    {
        ...
        $this->allowedObjectTypes = ['linker', 'carouselItem', 'websiteImage'];
    ...
    public function performFileUpload($objectType, $file, $fieldName)
    {
        ...
        try {
            ...
            $this->logger->error('{app}: User {user} could not detect upload destination path for entity {entity} and field {field}.', ['app' => 'RKWebsiteHelperModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $objectType, 'field' => $fieldName]);
        ...
        if ($isImage) {
            ...
            if (!is_array($imgInfo) || !$imgInfo[0] || !$imgInfo[1]) {
                ...
                $this->logger->error('{app}: User {user} tried to upload a file which is seems not to be a valid image.', ['app' => 'RKWebsiteHelperModule', 'user' => $this->currentUserApi->get('uname')]);
        ...
        if ($isImage) {
            ...
            if (true === $this->variableApi->get('RKWebsiteHelperModule', 'enableShrinkingFor' . $fieldSuffix, false)) {
                // check for maximum size
                $maxWidth = $this->variableApi->get('RKWebsiteHelperModule', 'shrinkWidth' . $fieldSuffix, 800);
                $maxHeight = $this->variableApi->get('RKWebsiteHelperModule', 'shrinkHeight' . $fieldSuffix, 600);
                ...
                if ($imgInfo[0] > $maxWidth || $imgInfo[1] > $maxHeight) {
                    ...
                    $preset = new \SystemPlugin_Imagine_Preset('RKWebsiteHelperModule_Shrinker', [
                        'width' => $maxWidth,
                        'height' => $maxHeight,
                        'mode' => 'inset'
                    ]);
    ...
    protected function validateFileUpload($objectType, $file, $fieldName)
    {
        ...
        if ($file->getError() != UPLOAD_ERR_OK) {
            ...
            $this->logger->error('{app}: User {user} tried to upload a file with errors: ' . $file->getErrorMessage(), ['app' => 'RKWebsiteHelperModule', 'user' => $this->currentUserApi->get('uname')]);
        ...
        if (false === $isValidExtension) {
            ...
            $this->logger->error('{app}: User {user} tried to upload a file with a forbidden extension ("{extension}").', ['app' => 'RKWebsiteHelperModule', 'user' => $this->currentUserApi->get('uname'), 'extension' => $extension]);
    ...
    {
        $basePath = $this->dataDirectory . '/RKWebsiteHelperModule/';
        ...
        switch ($objectType) {
            ...
            case 'carouselItem':
                ...
                break;
            case 'websiteImage':
                $basePath .= 'websiteimages/myimage/';
                break;
            default:
    ...
    public function checkAndCreateAllUploadFolders()
    {
        ...
        $result &= $this->checkAndCreateUploadFolder('websiteImage', 'myImage', 'gif, jpeg, jpg, png');
    ...
    protected function checkAndCreateUploadFolder($objectType, $fieldName, $allowedExtensions = '')
    {
        ...
        if (!$fs->exists($uploadPath)) {
            try {
                ...
                $this->logger->error('{app}: The upload directory {directory} does not exist and could not be created.', ['app' => 'RKWebsiteHelperModule', 'directory' => $uploadPath]);
        ...
        if (!is_writable($uploadPath)) {
            try {
                ...
                $this->logger->error('{app}: The upload directory {directory} exists but is not writable by the webserver.', ['app' => 'RKWebsiteHelperModule', 'directory' => $uploadPath]);
        ...
        $htaccessFileTemplate = 'modules/RK/WebsiteHelperModule/Resources/docs/htaccessTemplate';
        if (!$fs->exists($htaccessFilePath) && $fs->exists($htaccessFileTemplate)) {
            try {
                ...
                $this->logger->error('{app}: An error occured during creation of the .htaccess file in directory {directory}.', ['app' => 'RKWebsiteHelperModule', 'directory' => $uploadPath]);
            }
        }
