...
<p>
    <label for="{$baseID}Id"{$leftSide}>{gt text='Carousel'}:</label>
    <select id="{$baseID}Id" name="id"{$rightSide}>
        {foreach item='carousel' from=$items}
            <option value="{$carousel.id}"{if $selectedId eq $carousel.id} selected="selected"{/if}>{$carousel->getTitleFromDisplayPattern()}</option>
        {foreachelse}
...
<p>
    ...
    <select id="{$baseID}Sort" name="sort"{$rightSide}>
        ...
        <option value="carouselName"{if $sort eq 'carouselName'} selected="selected"{/if}>{gt text='Carousel name'}</option>
        <option value="remarks"{if $sort eq 'remarks'} selected="selected"{/if}>{gt text='Remarks'}</option>
        <option value="slidingTime"{if $sort eq 'slidingTime'} selected="selected"{/if}>{gt text='Sliding time'}</option>
        <option value="controls"{if $sort eq 'controls'} selected="selected"{/if}>{gt text='Controls'}</option>
        <option value="carouselGroup"{if $sort eq 'carouselGroup'} selected="selected"{/if}>{gt text='Carousel group'}</option>
