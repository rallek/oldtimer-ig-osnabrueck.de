...
abstract class AbstractUserListener implements EventSubscriberInterface
{
    ...
    public function delete(GenericEvent $event)
    {
        ...
        $repo = $entityManager->getRepository('RK\WebsiteHelperModule\Entity\LinkerEntity');
        ...
        $logArgs = ['app' => 'RKWebsiteHelperModule', 'user' => $serviceManager->get('zikula_users_module.current_user')->get('uname'), 'entities' => 'linkers'];
        $logger->notice('{app}: User {user} has been deleted, so we deleted/updated corresponding {entities}, too.', $logArgs);
        
        $repo = $entityManager->getRepository('RK\WebsiteHelperModule\Entity\CarouselItemEntity');
        ...
        $logArgs = ['app' => 'RKWebsiteHelperModule', 'user' => $serviceManager->get('zikula_users_module.current_user')->get('uname'), 'entities' => 'carousel items'];
        $logger->notice('{app}: User {user} has been deleted, so we deleted/updated corresponding {entities}, too.', $logArgs);
        
        $repo = $entityManager->getRepository('RK\WebsiteHelperModule\Entity\CarouselEntity');
        ...
        $logArgs = ['app' => 'RKWebsiteHelperModule', 'user' => $serviceManager->get('zikula_users_module.current_user')->get('uname'), 'entities' => 'carousells'];
        $logger->notice('{app}: User {user} has been deleted, so we deleted/updated corresponding {entities}, too.', $logArgs);
        
        $repo = $entityManager->getRepository('RK\WebsiteHelperModule\Entity\WebsiteImageEntity');
        // set creator to admin (2) for all website images created by this user
        $repo->updateCreator($uid, 2, $translator, $logger, $currentUserApi);
        
        // set last editor to admin (2) for all website images updated by this user
        $repo->updateLastEditor($uid, 2, $translator, $logger, $currentUserApi);
        
        $logArgs = ['app' => 'RKWebsiteHelperModule', 'user' => $serviceManager->get('zikula_users_module.current_user')->get('uname'), 'entities' => 'website images'];
        $logger->notice('{app}: User {user} has been deleted, so we deleted/updated corresponding {entities}, too.', $logArgs);
    }
}
