...
abstract class AbstractCarouselController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        $objectType = 'carousel';
        $utilArgs = ['controller' => 'carousel', 'action' => 'index'];
        ...
        if ($isAdmin) {
            ...
            return $this->redirectToRoute('rkwebsitehelpermodule_carousel_' . ($isAdmin ? 'admin' : '') . 'view');
        }
        ...
        return $this->render('@RKWebsitehelperModule/Carousel/index.html.twig', $templateParameters);
    }
    /**
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'carousel';
        $utilArgs = ['controller' => 'carousel', 'action' => 'view'];
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'rkwebsitehelpermodule_carousel_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
        ...
        $currentUrlObject = new RouteUrl('rkwebsitehelpermodule_carousel_' . /*($isAdmin ? 'admin' : '') . */'view', $currentUrlArgs);
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'carousel';
        $utilArgs = ['controller' => 'carousel', 'action' => 'edit'];
        ...
        $formHandler = $this->get('rk_websitehelper_module.form.handler.carousel');
     ...
     * @ParamConverter("carousel", class="RKWebsitehelperModule:CarouselEntity", options={"id" = "id", "repository_method" = "selectById"})
     * @Cache(lastModified="carousel.getUpdatedDate()", ETag="'Carousel' ~ carousel.getid() ~ carousel.getUpdatedDate().format('U')")
     *
     * @param Request  $request      Current request instance
    ...
    protected function deleteInternal(Request $request, CarouselEntity $carousel, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'carousel';
        $utilArgs = ['controller' => 'carousel', 'action' => 'delete'];
        ...
        $logArgs = ['app' => 'RKWebsitehelperModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'carousel', 'id' => $entity->createCompositeIdentifier()];
        ...
        if ($isAdmin) {
            ...
            $redirectRoute = 'rkwebsitehelpermodule_carousel_' . ($isAdmin ? 'admin' : '') . 'view';
        } else {
            // redirect to the list of carousells
            $redirectRoute = 'rkwebsitehelpermodule_carousel_' . ($isAdmin ? 'admin' : '') . 'view';
        }
     ...
     */
    public function adminDisplayAction(Request $request, CarouselEntity $carousel)
    {
        return $this->displayInternal($request, $carousel, true);
    }
     ...
     * @throws NotFoundHttpException Thrown by param converter if item to be displayed isn't found
     */
    public function displayAction(Request $request, CarouselEntity $carousel)
    {
        return $this->displayInternal($request, $carousel, false);
    }
    
    /**
     * This method includes the common implementation code for adminDisplay() and display().
     */
    protected function displayInternal(Request $request, CarouselEntity $carousel, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'carousel';
        $utilArgs = ['controller' => 'carousel', 'action' => 'display'];
        $permLevel = $isAdmin ? ACCESS_ADMIN : ACCESS_READ;
        if (!$this->hasPermission($this->name . ':' . ucfirst($objectType) . ':', '::', $permLevel)) {
            throw new AccessDeniedException();
        }
        ...
        $currentUrlArgs['_locale'] = $request->getLocale();
        $currentUrlObject = new RouteUrl('rkwebsitehelpermodule_carousel_' . /*($isAdmin ? 'admin' : '') . */'display', $currentUrlArgs);
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemid) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'RKWebsitehelperModule', 'user' => $userName, 'action' => $action, 'entity' => 'carousel', 'id' => $itemid, 'errorMessage' => $e->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'RKWebsitehelperModule', 'user' => $userName, 'entity' => 'carousel', 'id' => $itemid]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'RKWebsitehelperModule', 'user' => $userName, 'action' => $action, 'entity' => 'carousel', 'id' => $itemid]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('rkwebsitehelpermodule_carousel_' . /*($isAdmin ? 'admin' : '') . */'display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('rkwebsitehelpermodule_carousel_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
